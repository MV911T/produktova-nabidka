...
(viz 14), LAUF proběhl jako zkouška po opravě 20 — výstup ale zatím nikde
uložený není. Chybí:
...
už `ziskej_nabidku()` řeší (viz 24), takže stačí předat všechny čtyři URL
najednou a výstup uložit.

### Otevřené otázky

**3. Psyllium — semena vs. slupky**
...
**5.** Parsování pěti polí z produktové stránky — microdata + `.p-short-description`
**6.** Sběr URL z výpisu kategorie včetně stránkování (`/strana-N/`)
**7.** `category` z nadpisu výpisu — na produktu spolehlivá není
**8.** Přeskakování zrušených produktů — web je přesměruje na kategorii
**9.** Rozpoznání a vyřazení balíčků — tři signály plus ruční seznam
**10.** Běh na kategorii doplňků stravy — 364 produktů, 0 prázdných polí

### Popisy a legislativa

**11.** 14 vlastních popisů pro produkty, u nichž e-shop krátký popis nemá
**12.** Kontrola tvrzení proti Vodítkům SZPI 2024 — tři vrstvy (dnes čtyři, viz 22):
riziková léčebná slova, zesilující slovesa, zdravotní téma bez opory
**13.** Vyhledávač schválených a on-hold tvrzení podle látky
**14.** Chybějící popisy v kategoriích muži a ženy — psát se nemusel žádný.
...
**15.** Repozitář na GitHubu — [MV911T/produktova-nabidka](https://github.com/MV911T/produktova-nabidka)
**16.** Hlášky na stderr místo do JSON výstupu
**17.** Rozdělení do modulů, `pyproject.toml`, CLI, GitHub Actions
**18.** 79 testů bez přístupu na síť, `ruff` bez nálezů
**19.** Tabule úkolů po každé iteraci — `nastroje/tabule.py` a `Stop` hook.
...
**20.** Neúplná nabídka se pozná. Stahování opakuje pokus u výpadku spojení
...
**21.** Ašvaganda Sensoril® je z kořene **i listů** — technický list výrobce
...
**22.** Přesnost kontroly tvrzení. Na 165 popisech z e-shopu spadly
...
**23.** Provozní sdělení zmizí z popisu, ať končí čímkoli. Vzor žádal
...
**24.** Každý produkt je v nabídce jednou. E-shop vede `Men Multivitamin®`
...
**25.** Zdroj `category` prověřen a ponechán u nadpisu výpisu. Produkt
...
**26.** Celý katalog projet kontrolou tvrzení — 466 produktů ze všech čtyř
...
**27.** Náhradní znění pro texty, které kontrola označila. Deset míst
...
**28.** Rozpoznávání balíčků prověřeno proti celému katalogu. Ruční seznam
...
**29.** Kontrola mlčí tam, kde slovo má i samotná opora. Riziková slova
...
**30.** Stabilní identifikátory ve výstupu. K pěti polím ze zadání přibylo
...
**31.** Z nabídky vypadlo zboží, které se nejí. Kategorie mužů, žen a LAUF
