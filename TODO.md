...
(viz 12), LAUF proběhl jako zkouška po opravě 18 — výstup ale zatím nikde
uložený není. Chybí:
...
už `ziskej_nabidku()` řeší (viz 22), takže stačí předat všechny čtyři URL
...
**3.** Parsování pěti polí z produktové stránky — microdata + `.p-short-description`
**4.** Sběr URL z výpisu kategorie včetně stránkování (`/strana-N/`)
**5.** `category` z nadpisu výpisu — na produktu spolehlivá není
**6.** Přeskakování zrušených produktů — web je přesměruje na kategorii
**7.** Rozpoznání a vyřazení balíčků — tři signály plus ruční seznam
**8.** Běh na kategorii doplňků stravy — 364 produktů, 0 prázdných polí

### Popisy a legislativa

**9.** 14 vlastních popisů pro produkty, u nichž e-shop krátký popis nemá
**10.** Kontrola tvrzení proti Vodítkům SZPI 2024 — tři vrstvy (dnes pět, viz 20 a 31):
riziková léčebná slova, zesilující slovesa, zdravotní téma bez opory
**11.** Vyhledávač schválených a on-hold tvrzení podle látky
**12.** Chybějící popisy v kategoriích muži a ženy — psát se nemusel žádný.
...
**13.** Repozitář na GitHubu — [MV911T/produktova-nabidka](https://github.com/MV911T/produktova-nabidka)
**14.** Hlášky na stderr místo do JSON výstupu
**15.** Rozdělení do modulů, `pyproject.toml`, CLI, GitHub Actions
**16.** 79 testů bez přístupu na síť, `ruff` bez nálezů
**17.** Tabule úkolů po každé iteraci — `nastroje/tabule.py` a `Stop` hook.
...
**18.** Neúplná nabídka se pozná. Stahování opakuje pokus u výpadku spojení
...
**19.** Ašvaganda Sensoril® je z kořene **i listů** — technický list výrobce
...
**20.** Přesnost kontroly tvrzení. Na 165 popisech z e-shopu spadly
...
**21.** Provozní sdělení zmizí z popisu, ať končí čímkoli. Vzor žádal
...
**22.** Každý produkt je v nabídce jednou. E-shop vede `Men Multivitamin®`
...
**23.** Zdroj `category` prověřen a ponechán u nadpisu výpisu. Produkt
...
**24.** Celý katalog projet kontrolou tvrzení — 466 produktů ze všech čtyř
...
**25.** Náhradní znění pro texty, které kontrola označila. Deset míst
...
**26.** Rozpoznávání balíčků prověřeno proti celému katalogu. Ruční seznam
...
**27.** Kontrola mlčí tam, kde slovo má i samotná opora. Riziková slova
...
**28.** Stabilní identifikátory ve výstupu. K pěti polím ze zadání přibylo
...
**29.** Z nabídky vypadlo zboží, které se nejí. Kategorie mužů, žen a LAUF
...
**30.** Psyllium má vlastní on-hold tvrzení pro slupky, semena nebylo
...
**31.** Kontrola používá oficiální nepřípustná znění z Vodítek. Otázka
...
**32.** Vztah k účinku se pozná i bez slovesa. Brána na slovesa míjela
...
**33.** Seznam příznaků má doložený původ. Rozdělil se na `SYMPTOMY_Z_VODITEK`
...
**34.** Slovo se najde i ve skloněném tvaru. Hledání uměla jen přílepek, takže
