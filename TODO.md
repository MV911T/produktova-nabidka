...
(viz 19), LAUF proběhl jako zkouška po opravě 25 — výstup ale zatím nikde
uložený není. Chybí:
...
**3. Duplicita produktu ve výpisu jedné kategorie**
...
**4. `category` se bere z výpisu, ačkoli produkt vlastní kategorii nese**
...
**7. Má výstup nést stabilní identifikátor?**
...
**10.** Parsování pěti polí z produktové stránky — microdata + `.p-short-description`
**11.** Sběr URL z výpisu kategorie včetně stránkování (`/strana-N/`)
**12.** `category` z nadpisu výpisu — na produktu spolehlivá není
**13.** Přeskakování zrušených produktů — web je přesměruje na kategorii
**14.** Rozpoznání a vyřazení balíčků — tři signály plus ruční seznam
**15.** Běh na kategorii doplňků stravy — 364 produktů, 0 prázdných polí

### Popisy a legislativa

**16.** 14 vlastních popisů pro produkty, u nichž e-shop krátký popis nemá
**17.** Kontrola tvrzení proti Vodítkům SZPI 2024 — tři vrstvy:
riziková léčebná slova, zesilující slovesa, zdravotní téma bez opory
**18.** Vyhledávač schválených a on-hold tvrzení podle látky
**19.** Chybějící popisy v kategoriích muži a ženy — psát se nemusel žádný.
...
**22.** Rozdělení do modulů, `pyproject.toml`, CLI, GitHub Actions
**23.** 56 testů bez přístupu na síť, `ruff` bez nálezů
**24.** Tabule úkolů po každé iteraci — `nastroje/tabule.py` a `Stop` hook.
...
**25.** Neúplná nabídka se pozná. Stahování opakuje pokus u výpadku spojení
...
**26.** Ašvaganda Sensoril® je z kořene **i listů** — technický list výrobce
...
**27.** Přesnost kontroly tvrzení. Na 165 popisech z e-shopu spadly
...
**28.** Provozní sdělení zmizí z popisu, ať končí čímkoli. Vzor žádal
